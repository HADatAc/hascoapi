...
public class IngestionAPI extends Controller {
    ...
    public Result mtGenByElement(String elementtype, String datafileuri, String elementuri, String filename, String mediaFolder, String verifyUri, Boolean generateDASOCs, Boolean includeWorkflowModel) {
        ...
        System.out.println("  generateDASOCs: [" + generateDASOCs + "]");
        System.out.println("  includeWorkflowModel: [" + includeWorkflowModel + "]");
        
        // Default to false if not provided
        boolean shouldGenerateDASOCs = (generateDASOCs != null && generateDASOCs);
        // Default to true for backward compatibility.
        boolean shouldIncludeWorkflowModel = (includeWorkflowModel == null || includeWorkflowModel);
        ...
        ProcessBasedStudy selectedProcessBasedStudy = null;
        if ("dsg".equals(elementtype)) {
            selectorType = "study";
        } else if ("ins".equals(elementtype)) {
            selectorType = "instrument";
        } else if ("wkf".equals(elementtype)) {
            // Prefer scenario-specific generation when a ProcessBasedStudy URI was provided.
            selectedProcessBasedStudy = ProcessBasedStudy.find(elementuri);
            if (selectedProcessBasedStudy == null) {
                // Some callers may pass Study ID instead of URI.
                Study candidateStudy = Study.findById(elementuri);
                if (candidateStudy != null && candidateStudy.getUri() != null && !candidateStudy.getUri().isEmpty()) {
                    selectedProcessBasedStudy = ProcessBasedStudy.find(candidateStudy.getUri());
                }
            }
            selectorType = (selectedProcessBasedStudy != null) ? "processbasedstudy" : "processstem";
        ...
        } else if (selectorType.equals("processbasedstudy")) {
            element = selectedProcessBasedStudy;
        ...
        if ("processbasedstudy".equals(selectorType)) {
            String normalized = deriveWkfFilenameFromProcessBasedStudyUri(elementuri);
            if (normalized != null && !normalized.isEmpty()) {
                System.out.println("  WKF filename normalized from ProcessBasedStudy URI: [" + filename + "] -> [" + normalized + "]");
                filename = normalized;
            }
        }
        ...
        switch (selectorType) {
            ...
            case "processbasedstudy":
                resp = WKFGen.genByProcessBasedStudy((ProcessBasedStudy) element, filename, mediaFolder, verifyUri, shouldIncludeWorkflowModel);
                break;
            case "processstem":
                resp = WKFGen.genByProcessStem((ProcessStem)element,filename,mediaFolder,verifyUri,shouldIncludeWorkflowModel);
                break;
            case "organization":
    ...
    private String deriveWkfFilenameFromProcessBasedStudyUri(String uri) {
        if (uri == null || uri.trim().isEmpty()) {
            return null;
        }

        String localId = uri.trim();
        int slash = localId.lastIndexOf('/');
        int hash = localId.lastIndexOf('#');
        int cut = Math.max(slash, hash);
        if (cut >= 0 && cut + 1 < localId.length()) {
            localId = localId.substring(cut + 1);
        }
        ...
        if (localId.regionMatches(true, 0, "STD-", 0, 4)) {
            localId = "WKF-" + localId.substring(4);
        } else if (!localId.regionMatches(true, 0, "WKF-", 0, 4)) {
            localId = "WKF-" + localId;
        }

        if (!localId.toLowerCase().endsWith(".xlsx")) {
            localId = localId + ".xlsx";
        }
